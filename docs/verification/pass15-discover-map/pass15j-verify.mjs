#!/usr/bin/env node
/** Pass15j — branded lagoon water + distinctive surface + tap flow */
...
const SS = path.join(ROOT, 'screenshots', 'pass15j');
const LOG = path.join(ROOT, 'pass15j-verify-log.jsonl');
...
async function main() {
  ...
  try {
    ...
    await d.pause(6000);
    await shot(d, '01-map-surface-colombo-water.png');
    ...
    try {
      const markers = await d.$$('-ios predicate string:name BEGINSWITH "AIRGMSMarker"');
      if (markers.length > 0) {
    ...
    try {
      ...
      if (await preview.isDisplayed().catch(() => false)) {
        ...
        results.previewOpensOutlet =
          outletSrc.includes('Outlet') &&
          (outletSrc.includes('Bakehouse') ||
            outletSrc.includes('clearance shelf') ||
            outletSrc.includes('Active rescue bags'));
      } else {
