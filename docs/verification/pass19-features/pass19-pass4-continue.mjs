#!/usr/bin/env node
/** Pass4 continue — rows after B-07 (session already authenticated) */
...
const log = (e) =>
  fs.appendFileSync(LOG, JSON.stringify({ ts: new Date().toISOString(), wave: 'pass4', ...e }) + '\n');
const shot = async (d, name) => {
  fs.mkdirSync(SS, { recursive: true });
  fs.writeFileSync(path.join(SS, name), Buffer.from(await d.takeScreenshot(), 'base64'));
  return `screenshots/pass19/pass4/${name}`;
};
async function tryTap(d, pred) {
  try {
...
try {
  ...
  R['B-15'] = /Prices refreshed/i.test(b15Src);
  log({ id: 'B-15', tool: 'appium.journey', result_summary: R['B-15'] ? 'PASS' : 'PARTIAL', evidence: await shot(d, 'B-15-basket-expired.png') });
  ...
  await d.pause(2000);
  R['D-03'] = /Green Grocer|supermarket|Browse shelf/i.test(await d.getPageSource());
  log({ id: 'D-03', tool: 'appium.screenshot', result_summary: R['D-03'] ? 'PASS' : 'PARTIAL', evidence: await shot(d, 'D-03-supermarket-no-pulse.png') });
  ...
  if (R['D-06']) {
    ...
    await d.pause(3500);
    R['M4-3'] = /View bags|Reserve|Clearance|Bakehouse/i.test(await d.getPageSource());
  ...
  if (await addPhoto.isDisplayed().catch(() => false)) await addPhoto.click();
  else await tryTap(d, 'label == "Add a photo"');
  await d.pause(2000);
  await tryTap(d, 'label CONTAINS "Photo Library"');
  await d.pause(2000);
  ...
  await tryTap(d, 'label CONTAINS "Share your impact" OR label CONTAINS "Share"');
  await d.pause(2500);
  ...
  log({ id: 'M3', tool: 'appium.journey', result_summary: R['M3'] ? 'PASS' : 'PARTIAL', evidence: await shot(d, 'M3-story-share-sheet.png') });
  ...
  if (await rev.isDisplayed().catch(() => false)) {
    ...
    await tryTap(d, 'label CONTAINS "Checkout" OR label CONTAINS "Continue to checkout"');
    await d.pause(4000);
    R['M2'] = /Payment Method|Checkout|Pay at Store|Card Payment/i.test(await d.getPageSource());
    await shot(d, 'M2-3-shelf-checkout.png');
  }
  log({ id: 'M2', tool: 'appium.journey', result_summary: R['M2'] ? 'PASS' : 'PARTIAL', evidence: 'screenshots/pass19/pass4/M2-3-shelf-checkout.png' });

  injectAsyncStorage(CART_KEY, {
    outletId: BAKEHOUSE_OUTLET,
    bagIds: [BAG_B],
    bags: [{ id: BAG_B, outletId: BAKEHOUSE_OUTLET, title: 'Surprise Pastries', rescuePrice: 600 }],
  });
  await dl(`freshasever://checkout?group=${BAG_B}`);
  await d.pause(5000);
  await shot(d, 'M1-1-group-checkout.png');
  R['M1'] = /Pay at Store|Card Payment|checkout.groupStrip/i.test(await d.getPageSource());
  if (R['M1']) {
    await tryTap(d, 'label CONTAINS "Card Payment"');
    await d.pause(1500);
    await shot(d, 'M1-2-card-selected.png');
    await tryTap(d, 'label CONTAINS "Pay at Store"');
    await d.pause(1500);
    await shot(d, 'M1-2-pay-at-store.png');
  }
  log({ id: 'M1', tool: 'appium.journey', result_summary: R['M1'] ? 'PASS' : 'PARTIAL', evidence: 'screenshots/pass19/pass4/M1-1-group-checkout.png' });
